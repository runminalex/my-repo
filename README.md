...
A simple "Hello, World!" website served via Python's built-in HTTP server, with a scientific calculator module, CLI, statistics module, and correlation/regression analysis.
...
3. **Statistics Module** — a Python module for descriptive statistical operations, correlation, and regression analysis on data sets.
...
### Available Descriptive Operations
...
The `Statistics` class also provides correlation and regression analysis between paired data sets.
...
# Population covariance (ddof=0, default)
print(stats.covariance(x, y))           # 4.8

# Sample covariance (ddof=1)
print(stats.covariance(x, y, ddof=1))   # 6.0
```

#### Pearson Correlation

The Pearson correlation coefficient measures linear correlation between two data sets, ranging from -1 (perfect negative) to +1 (perfect positive).
...
print(r)  # ~0.999 (very strong positive correlation)
```

#### Spearman Rank Correlation

The Spearman rank correlation assesses monotonic relationships using ranked data. It is useful for non-linear but monotonic relationships.

```python
# Monotonically increasing but non-linear
x = [1, 2, 3, 4, 5]
y = [1, 4, 9, 16, 25]  # y = x^2

r = stats.spearman_correlation(x, y)
print(r)  # 1.0 (perfect monotonic relationship)
```

#### Linear Regression

Performs simple linear regression to find the best-fit line y = slope * x + intercept.
...
print(result["intercept"])   # 1.0
print(result["r_squared"])   # 1.0 (perfect fit)
print(result["r"])           # 1.0 (perfect correlation)
```

### Correlation & Regression Operations Reference

| Operation                            | Description                                          | Raises                                                        |
|--------------------------------------|------------------------------------------------------|---------------------------------------------------------------|
| `covariance(x, y, ddof=0)`          | Covariance between two data sets                     | `ValueError` if lengths differ or insufficient data           |
| `pearson_correlation(x, y)`         | Pearson correlation coefficient (linear)             | `ValueError` if lengths differ, < 2 points, or zero variance  |
| `spearman_correlation(x, y)`        | Spearman rank correlation coefficient (monotonic)    | `ValueError` if lengths differ or < 2 points                  |
| `linear_regression(x, y)`           | Linear regression (returns slope, intercept, r^2, r) | `ValueError` if lengths differ, < 2 points, or x has no variance |
...
├── src/
│   ├── __init__.py            # Calculator & Statistics package init
│   ├── calculator.py          # Calculator module (basic + scientific)
│   ├── cli.py                 # Command-line interface
│   └── stats.py               # Statistics module (descriptive + correlation + regression)
├── tests/
│   ├── __init__.py            # Tests package init
│   ├── test_calculator.py     # Calculator tests (basic + scientific + CLI)
│   └── test_stats.py          # Statistics tests (descriptive + correlation + regression)
